...
15. Provider tokens remain server-side under the Backend for Frontend (BFF) architecture. The browser interacts solely through expiring, invalidatable application sessions with secure cookie semantics (`HttpOnly`, `SameSite=Lax`, `Secure`), session fixation rotation, and CSRF protection. Provider tokens and client secrets are never exposed to browser storage, JavaScript, error payloads, or logs.
